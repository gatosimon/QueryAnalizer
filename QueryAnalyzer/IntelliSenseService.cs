    ...
    public static class IntelliSenseService
    {
        ...
        private static List<ColumnInfo> GetColumnas(Conexion conexion, string tabla)
        {
            ...
            try
            {
                ...
                while (DB.Read())
                {
                    string nombre = DB.Reader[0].ToString().Trim();
                    string tipo = DB.Reader.FieldCount > 1 ? DB.Reader[1].ToString().Trim() : string.Empty;
                    ...
                    if (conexion.Motor == TipoMotor.SQLite)
                    {
                        ...
                        tipo = DB.Reader[2].ToString().Trim();
                    }
        ...
        private static List<TablaInfo> GetTablas(Conexion conexion)
        {
            ...
            try
            {
                ...
                if (string.IsNullOrEmpty(query))
                {
                    ...
                    DataTable schema = DB.Connection.GetSchema("Tables");
                ...
                {
                    string nombre = DB.Reader[0].ToString().Trim();
                    string esquema = DB.Reader.FieldCount > 1 ? DB.Reader[1].ToString().Trim() : string.Empty;
                    string tipo = DB.Reader.FieldCount > 2 ? DB.Reader[2].ToString().Trim() : "TABLE";
                    if (!string.IsNullOrEmpty(nombre))
                        resultado.Add(new TablaInfo { Nombre = nombre, Esquema = esquema, Tipo = tipo });
                }
            }
