...
namespace QueryAnalyzer
{
    public partial class MainWindow : Window
    {
            ...
            try
            {
                ...
                for (int i = 0; i < validQueries.Count; i++)
                {
                    ...
                    await Dispatcher.InvokeAsync(() =>
                    {
                        ...
                        var columnasClaveManual = new System.Collections.Generic.List<string>();
                        ...
                        dataGrid.AutoGeneratingColumn += (s, ev) =>
                        {
                            ...
                            if (esEditable)
                            {
                                var mnuMarcar = new MenuItem();
                                AplicarEstiloMenuItem(mnuMarcar);
                                ...
                                tbHeader.ContextMenu = headerMenu;
                            }
                        };
            ...
            string connStr, TipoMotor motor, string tableName)
        {
            ...
            try
            {
                ...
                if (motor == TipoMotor.SQLite)
                {
                    ...
                    // Intento 2: primer índice disponible (único preferido, no único como fallback)
                    DB.CommandText = $"PRAGMA index_list('{tablaSimple}')";
                    var indices = new System.Collections.Generic.List<(string name, int unique)>();
                    while (DB.Read())
                        indices.Add((DB.Reader["name"].ToString(), Convert.ToInt32(DB.Reader["unique"])));
                    DB.CloseConnection();

                    foreach (var idx in indices.OrderByDescending(i => i.unique))
                    {
                        var dbIdx = new DataBase(connStr, false);
                        dbIdx.CommandText = $"PRAGMA index_info('{idx.name}')";
                        var cols = new System.Collections.Generic.List<(int seqno, string name)>();
                        while (dbIdx.Read())
                            cols.Add((Convert.ToInt32(dbIdx.Reader["seqno"]), dbIdx.Reader["name"].ToString()));
                        dbIdx.CloseConnection();
                        if (cols.Count > 0)
                            return cols.OrderBy(c => c.seqno).Select(c => c.name).ToList();
                    }
                    return resultado;
                }
        ...
        private void ToggleColumnaClaveManual(string colName, System.Windows.Controls.TextBlock tbHeader,
            System.Collections.Generic.List<string> columnasClaveManual)
        {
            bool esClave = columnasClaveManual.Contains(colName, StringComparer.OrdinalIgnoreCase);
            if (esClave)
            {
        ...
        private async void BtnGuardarCambios_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                // Columnas clave: primero la selección manual del usuario (clic derecho en encabezado),
                // si no, auto-detección desde el catálogo del motor (PK → índice único → índice no único).
                // Si no se encuentra nada, se cae de vuelta a comparar por todas las columnas.
                System.Collections.Generic.List<string> columnasClave;
                if (info.ColumnasClaveManual?.Count > 0)
                    columnasClave = info.ColumnasClaveManual;
                else
                    columnasClave = await Task.Run(() =>
                ...
                await Task.Run(() =>
                {
                    ...
                    try
                    {
                        ...
                        if (filasSinAfectar > 0)
                        {
                            ...
                            throw new Exception(
                                $"{filasSinAfectar} fila(s) no encontradas en \"{tableName}\" " +
                                "(posible modificación concurrente). Se revirtieron todos los cambios.");
                        }
        ...
        private void ActualizarContadorSeleccion()
        {
            ...
            if (btnGenerarInserts != null)
                btnGenerarInserts.IsEnabled = total > 0;
        }
        ...
        private async void btnGenerarInserts_Click(object sender, RoutedEventArgs e)
        {
            if (conexionActual == null)
            {
            ...
            var nodos = GetNodosSeleccionados()
                .Where(n => (n.Tag as NodoTablaTag)?.Tipo == "TABLE")
                .ToList();
            ...
            try
            {
                ...
                string resultado = await System.Threading.Tasks.Task.Run(() =>
                {
                    ...
                    foreach (string nombreCompleto in nombresTablas)
                    {
                        ...
                        {
                            var DB = new DataBase(connStr, false);
                            DB.CommandText = $"SELECT * FROM {nombreCompleto}";
                            var dt = new DataTable();
                            DB.DataAdapter.Fill(dt);
                            DB.CloseConnection();
        ...
        private sealed class TabInfo
        {
            ...
            internal System.Collections.Generic.List<string> ColumnasClaveManual;
        }
    }
}
