    ...
    public static class DocumentadorService
    {
            ...
            Conexion conexion, string schema, string tabla, string tipo)
        {
            ...
            try
            {
                ...
                var pks = GetPKs(connStr, conexion.Motor, schema, tabla);
                var fks = GetFKs(connStr, conexion.Motor, schema, tabla);

                // Obtener descripciones de columnas y de la tabla/vista
                Dictionary<string, string> descsColumnas;
                string descTabla;
                GetDescripciones(connStr, conexion.Motor, schema, tabla,
                ...
                var cols = DB.GetSchema("Columns", new string[] { null, schema, tabla });
        ...
        private static HashSet<string> GetPKs(string connStr, TipoMotor motor, string schema, string tabla)
        {
            ...
            try
            {
                ...
                while (DB.Read())
                {
                    if (motor == TipoMotor.SQLite)
                    {
                        ...
                        int pkOrd = DB.GetOrdinal("pk");
                        int nameOrd = DB.GetOrdinal("name");
                        if (!DB.IsDBNull(pkOrd) && DB.GetInt32(pkOrd) > 0)
                            set.Add(DB.GetString(nameOrd));
                    }
                    else
                    {
                        set.Add(DB.Reader[0].ToString().Trim());
                    }
                }
                DB.CloseConnection();
            }
        ...
        private static HashSet<string> GetFKs(string connStr, TipoMotor motor, string schema, string tabla)
        {
            var set = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
            try
            {
                    ...
                    case TipoMotor.MS_SQL:
                        {
                            ...
                            while (DB.Read())
                            {
                                string desc = DB.IsDBNull(2) ? string.Empty : DB.Reader[2].ToString().Trim();
                                if (string.IsNullOrEmpty(desc)) continue;

                                int minorId = DB.IsDBNull(0) ? 0 : Convert.ToInt32(DB.Reader[0]);
                                if (minorId == 0)
                                {
                                    descripTabla = desc;
                                }
                                ...
                                {
                                    if (!DB.IsDBNull(1))
                                        descripsColumnas[DB.Reader[1].ToString().Trim()] = desc;
                                }
                            }
                            DB.CloseConnection();
                            break;
                    ...
                    case TipoMotor.POSTGRES:
                        {
                            ...
                            while (DB.Read())
                            {
                                ...
                                int subId = DB.IsDBNull(0) ? 0 : Convert.ToInt32(DB.Reader[0]);
                                if (subId == 0)
                                {
                                    descripTabla = desc;
                                }
                    ...
                    case TipoMotor.DB2:
                        {
                            ...
                            DataBase DB = new DataBase(connStr);
                            DB.CommandText = sqlCols;
                            while (DB.Read())
                            {
                                if (!DB.IsDBNull(1))
                                    descripsColumnas[DB.Reader[0].ToString().Trim()] = DB.Reader[1].ToString().Trim();
                            }
