            ...
            try
            {
                ...
                for (int i = 0; i < validQueries.Count; i++)
                {
                    ...
                    await Dispatcher.InvokeAsync(() =>
                    {
                        ...
                        // AutoGeneratingColumn: ev.PropertyName NO es confiable cuando los nombres
                        // de columna tienen guion bajo. WPF lo elimina al construir el property path
                        // del binding sobre DataRowView (COMUNA_J -> COMUNAJ).
                        // Solución: capturar el índice de generación y buscar la columna por posición.
                        int _autoColIdx = 0;
                        dataGrid.AutoGeneratingColumn += (s, ev) =>
                        {
                            // Obtener la columna real del DataTable por índice de generación
                            DataColumn dcReal = _autoColIdx < dt.Columns.Count ? dt.Columns[_autoColIdx] : null;
                            _autoColIdx++;

                            if (dcReal == null)
                                return;

                            // Forzar el header con el nombre real (preserva guiones bajos)
                            ev.Column.Header = dcReal.ColumnName;

                            Type tipo = dcReal.DataType;
...
////                            // Leemos los nombres reales de columna ANTES de usar el adapter,
////                            // porque algunos drivers ODBC (ej. DB2) eliminan los guiones bajos
////                            // al llenar el DataTable mediante Fill().
////                            string[] nombresReales = null;
////                            using (var reader = cmd.ExecuteReader(System.Data.CommandBehavior.SchemaOnly))
////                            {
////                                nombresReales = new string[reader.FieldCount];
////                                for (int i = 0; i < reader.FieldCount; i++)
////                                    nombresReales[i] = reader.GetName(i);
////                            }
...
////                            // Restaurar los nombres reales si el driver los alteró (ej: quitó guiones bajos)
////                            if (nombresReales != null && nombresReales.Length == dt.Columns.Count)
////                            {
////                                for (int i = 0; i < dt.Columns.Count; i++)
////                                {
////                                    if (dt.Columns[i].ColumnName != nombresReales[i])
////                                        dt.Columns[i].ColumnName = nombresReales[i];
////                                }
////                            }
////                        }
////                    }
